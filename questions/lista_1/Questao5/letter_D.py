...
def letter_D():
    ...
    st.subheader(
        "d) O box-plot para cada prova e comente as diferenças ou as semelhanças."
    )
    boxplot_data = midpoints_df[["1a", "2a", "3a", "4a", "Optativa"]]
    fig = px.box(boxplot_data)
    st.plotly_chart(fig)
    st.write(
        "A 1a teve uma média parecida com a 4a, porém apenas a 2a e 4a obtiveram notas 10"
    )
